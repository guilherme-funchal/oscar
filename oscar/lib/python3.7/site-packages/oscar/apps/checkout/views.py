...
class PaymentDetailsView(OrderPlacementMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        #Realiza Onboard no Aries
            count_user_order = Order.objects.filter(user_id=self.checkout_session.request._cached_user.id).count()
            email_user = self.request.user.email
            
            if count_user_order == 0:
                result = on_board_aries(email_user)
                
            # result = send_badge(email_user)
