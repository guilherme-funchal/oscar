...
class AbstractBasket(models.Model):
    ...
    securitys = models.ManyToManyField(
        'security.Security', verbose_name=_("Securitys"), blank=True)
    ...
    def merge(self, basket, add_quantities=True):
        ...
        for security in basket.securitys.all():
            basket.securitys.remove(security)
            self.securitys.add(security)
    ...
    @property
    def contains_a_security(self):
        if not self.id:
            return False
        return self.securitys.exists()
    ...
    def contains_security(self, code):
        if self.id is None:
            return False
        try:
            self.securitys.get(code=code)
        except ObjectDoesNotExist:
            return False
        else:
            return True
