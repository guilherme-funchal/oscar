...
from oscar.apps.basket.signals import (
    basket_addition, voucher_addition, voucher_removal, security_addition, security_removal)
...
(BasketLineForm, AddToBasketForm, BasketVoucherForm, BasketSecurityForm, SavedLineForm) = get_classes(
    'basket.forms', ('BasketLineForm', 'AddToBasketForm',
                     'BasketVoucherForm', 'BasketSecurityForm', 'SavedLineForm'))
...
Security = get_model('basket', 'basket_securitys')
...
class BasketView(ModelFormSetView):
    ...
    def get_basket_security_form(self):
        return BasketSecurityForm()

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['voucher_form'] = self.get_basket_voucher_form()
        context['security_form'] = self.get_basket_security_form()
        ...
        user_id = context['view'].request.session._session_cache['_auth_user_id']
                              
        count = Security.objects.filter(basket_id=basket_id).count()
        count_user_order = Order.objects.filter(user_id=user_id).count()
...
class SecurityAddView(FormView):
    form_class = BasketSecurityForm
    security_model = get_model('security', 'security')
    add_signal = security_addition
    ...
    def form_valid(self, form):
        ...
        if not self.request.basket.id:
            return redirect_to_referrer(self.request, 'basket:summary')
        if self.request.basket.contains_security(code):
                      
            messages.error(
                self.request,
                _("You have already added the '%(code)s' security to "
                  "your basket") % {'code': code})
        else:
            basket_id = self.request.basket.id
            
            test_code = check_code(self.request.user.email, code, basket_id)
            
            if test_code is not None:
                messages.success(
                    self.request,
                    _("Security found with code '%(code)s'") % {
                        'code': code})
            ...
            if test_code is None:
                messages.error(
                    self.request,
                    _("No security found with code '%(code)s'") % {
                        'code': code})
                       
        return redirect_to_referrer(self.request, 'basket:summary')
        
    def form_invalid(self, form):
        messages.error(self.request, _("Please enter a security code"))
        return redirect(reverse('basket:summary') + '#security')
...
class SecurityRemoveView(View):
    security_model = get_model('security', 'security')
    remove_signal = security_removal
    http_method_names = ['post']
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not request.basket.id:
            return response
        try:
            security = request.basket.vsecuritys.get(id=security_id)
        except ObjectDoesNotExist:
            messages.error(
                request, _("No security found with id '%s'") % security_id)
        else:
            request.basket.securitys.remove(security)
            self.remove_signal.send(
                sender=self, basket=request.basket, security=security)
            messages.info(
                request, _("Security '%s' removed from basket") % security.code)
