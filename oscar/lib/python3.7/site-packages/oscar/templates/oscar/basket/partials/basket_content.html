...
{% if not basket.is_empty %}
    ...
    {% block securitys %}
              {% if security_form %}
                <div class="col-sm-6">
                    <div class="sub-header">
                        <h2>{% trans "Security code" %}</h2>
                    </div>
                    <p id="security_form_link"><a href="#security" class="btn btn-secondary btn-full">{% trans "I have a security code..." %}</a></p>

                    <div id="security_form_container" style="display:none">
                        <form id="security_form" action="{% url 'basket:securitys-add' %}" method="post">
                            {% csrf_token %}
                            {% include "oscar/partials/form_fields.html" with form=security_form %}
                            <button type="submit" class="btn btn-info" data-loading-text="{% trans 'Adding...' %}">{% trans "Add security" %}</button>
                            {% trans "or" %} <a href="#" id="security_form_cancel">{% trans "cancel" %}</a>
                        </form>
                    </div>
                </div>
            {% endif %}
	...
	{% comment %} <div class="form-actions">
    		{% if anon_checkout_allowed or request.user.is_authenticated %}
        		<a href="{% url 'paypal-redirect' %}"><img src="https://www.paypal.com/pt_BR/i/btn/btn_xpressCheckout.gif" align="left" style="margin-right:7px;"></a>
    		{% endif %}
